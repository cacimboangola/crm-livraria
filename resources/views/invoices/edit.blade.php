...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h2">Editar Fatura #{{ $invoice->id }}</h1>
        <div class="btn-group" role="group">
            <a href="{{ route('invoices.index') }}" class="btn btn-outline-secondary">
                <i class="bi bi-arrow-left"></i> Voltar
            </a>
            <a href="{{ route('invoices.show', $invoice->id) }}" class="btn btn-outline-info">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="date" class="form-control @error('due_date') is-invalid @enderror" id="due_date" name="due_date" value="{{ old('due_date', $invoice->due_date ? $invoice->due_date->format('Y-m-d') : '') }}">
                                @error('due_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                                            ...
                                            <tr class="item-row">
                                                <td>
                                                    <select class="form-select book-select" name="items[{{ $index }}][book_id]" required>
                                                        ...
                                                        @foreach($books as $book)
                                                            <option value="{{ $book->id }}" data-price="{{ $book->price }}" data-stock="{{ $book->stock }}" {{ $item->book_id == $book->id ? 'selected' : '' }}>
                                                ...
                                                <td>
                                                    <div class="input-group">
                                                        ...
                                                        <input type="number" class="form-control price-input" name="items[{{ $index }}][unit_price]" value="{{ $item->unit_price }}" step="0.01" min="0" required>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div class="input-group">
                                                        <span class="input-group-text">Kz</span>
                                                        <input type="text" class="form-control item-total" value="{{ number_format($item->quantity * $item->unit_price, 2, '.', '') }}" readonly>
                                                    </div>
                                                </td>
                                                <td>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between mb-2">
                            <span>Subtotal:</span>
                            <span id="subtotal">Kz {{ number_format($invoice->subtotal, 2, ',', '.') }}</span>
                        </div>
                        ...
                        <div class="d-flex justify-content-between mb-4">
                            <h5>Total:</h5>
                            <h5 id="total">Kz {{ number_format($invoice->total, 2, ',', '.') }}</h5>
                        </div>
                        
                        <input type="hidden" name="subtotal" id="subtotal_input" value="{{ $invoice->subtotal }}">
                        <input type="hidden" name="total" id="total_amount_input" value="{{ $invoice->total }}">
                        
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-success" id="saveInvoiceBtn">
                                <i class="bi bi-save"></i> Atualizar Fatura
                            </button>
                        </div>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Configurar eventos para linhas existentes
