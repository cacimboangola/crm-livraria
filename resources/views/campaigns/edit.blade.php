        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <span>{{ __('Editar Campanha de Marketing') }}</span>
                    <div>
                        <a href="{{ route('campaigns.index') }}" class="btn btn-sm btn-secondary">
                            <i class="fas fa-arrow-left"></i> {{ __('Voltar para Lista') }}
                        </a>
                        <a href="{{ route('campaigns.show', $campaign->id) }}" class="btn btn-sm btn-info">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('campaigns.update', $campaign->id) }}" method="POST">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                                        ...
                                        <option value="email" {{ old('type', $campaign->type) == 'email' ? 'selected' : '' }}>{{ __('Email') }}</option>
                                        <option value="sms" {{ old('type', $campaign->type) == 'sms' ? 'selected' : '' }}>{{ __('SMS') }}</option>
                                        <option value="desconto" {{ old('type', $campaign->type) == 'desconto' ? 'selected' : '' }}>{{ __('Desconto') }}</option>
                                        <option value="evento" {{ old('type', $campaign->type) == 'evento' ? 'selected' : '' }}>{{ __('Evento') }}</option>
                                    </select>
                                    @error('type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="datetime-local" class="form-control @error('start_date') is-invalid @enderror" id="start_date" name="start_date" value="{{ old('start_date', $campaign->start_date->format('Y-m-d\TH:i')) }}" required>
                                    @error('start_date')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="datetime-local" class="form-control @error('end_date') is-invalid @enderror" id="end_date" name="end_date" value="{{ old('end_date', $campaign->end_date ? $campaign->end_date->format('Y-m-d\TH:i') : '') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('content') is-invalid @enderror" id="content" name="content" rows="10" required>{{ old('content', $campaign->content) }}</textarea>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <input type="number" class="form-control" id="target_criteria_min_purchases" name="target_criteria[min_purchases]" value="{{ old('target_criteria.min_purchases', $campaign->target_criteria['min_purchases'] ?? '') }}" min="0">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                <label for="target_criteria[min_total_spent]" class="form-label">{{ __('Valor Mínimo Gasto (R$)') }}</label>
                                                <input type="number" class="form-control" id="target_criteria_min_total_spent" name="target_criteria[min_total_spent]" value="{{ old('target_criteria.min_total_spent', $campaign->target_criteria['min_total_spent'] ?? '') }}" min="0" step="0.01">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <input type="number" class="form-control" id="target_criteria_days_since_last_purchase" name="target_criteria[days_since_last_purchase]" value="{{ old('target_criteria.days_since_last_purchase', $campaign->target_criteria['days_since_last_purchase'] ?? '') }}" min="0">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select" id="target_criteria_categories" name="target_criteria[categories][]" multiple>
                                                    @foreach(\App\Models\BookCategory::all() as $category)
                                                        <option value="{{ $category->id }}" 
                                                            {{ in_array($category->id, old('target_criteria.categories', $campaign->target_criteria['categories'] ?? [])) ? 'selected' : '' }}>
                                                            {{ $category->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> {{ __('Atualizar Campanha') }}
                            </button>
                        </div>
                    </form>
                </div>
